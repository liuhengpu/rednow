...
public class MainRecyclerAdapter extends RecyclerView.Adapter<MainRecyclerAdapter.MainViewHolder> {
  ...
  public void onBindViewHolder(MainViewHolder holder, final int position) {
    ...
    Picasso.with(mContext).load(mList.get(position).getAVFile("image") == null ? "www" : mList.get(position).getAVFile("image").getUrl()).transform(new RoundedTransformation(20, 0)).into(holder.mPicture);

    LogUtil.log.e("liuheng",""+mList.get(position).getAVFile("image").getUrl());
